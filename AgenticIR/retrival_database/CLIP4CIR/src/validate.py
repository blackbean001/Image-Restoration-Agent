...
from data_utils import squarepad_transform, ImgResDataset, FashionIQDataset, targetpad_transform, CIRRDataset
from combiner import Combiner
from utils import extract_index_features, collate_fn, element_wise_sum, device

def compute_imgres_val_metrics(relative_val_dataset: ImgResDataset, clip_model: CLIP, index_features: torch.tensor, index_names: List[str], combining_function: callable) -> Tuple[
    ...
    predicted_features, reference_names, target_names = \
        generate_imgres_val_predictions(clip_model, relative_val_dataset, combining_function, index_names, index_features)

    print("Compute ImgRes validation metrics")

    # Normalize the index features
    index_features = F.normalize(index_features, dim=-1).float()
    
    # get type of index_names and target_names
    #index_names = [item.split("/")[-2] for item in index_names]
    #target_names = [item.split("/")[-2] for item in target_names]
    #print("index_names, target_names: ", index_names, target_names)
    ...
    return recall_at1, recall_at5, recall_at10, recall_at50


def generate_imgres_val_predictions(clip_model: CLIP, relative_val_dataset: ImgResDataset, combining_function: callable, index_names: List[str], index_features: torch.tensor) -> \
    ...
    """
    print("Compute ImgRes validation predictions")
    ...
    for batch_reference_names, batch_target_names, captions in tqdm(
...
def imgres_val_retrieval(combining_function: callable, clip_model: CLIP, preprocess: callable):
    """
    ...
    classic_val_dataset = ImgResDataset('val', 'classic', preprocess)
    index_features, index_names = extract_index_features(classic_val_dataset, clip_model)
    relative_val_dataset = ImgResDataset('val', 'relative', preprocess)

    return compute_cirr_val_metrics(relative_val_dataset, clip_model, index_features, index_names,
                                    combining_function)

"""
def compute_imgres_val_metrics(relative_val_dataset: ImgResDataset, clip_model: CLIP, index_features: torch.tensor,
    ...
    # Generate predictions
    predicted_features, target_names = generate_imgres_val_predictions(clip_model, relative_val_dataset,
...
def generate_imgres_val_predictions(clip_model: CLIP, relative_val_dataset: ImgResDataset,
    ...
    print(f"Compute ImageRes validation predictions")
        ...
        with torch.no_grad():
            ...
            if text_features.shape[0] == 1:
                ...
                reference_image_features = torch.stack(itemgetter(*reference_names)(
                    name_to_feat))  # To avoid unnecessary computation retrieve the reference image features directly from the index features
            #print("shapes: ", reference_image_features.shape, text_features.shape)
            batch_predicted_features = combining_function(reference_image_features, text_features)
...
def imgres_val_retrieval(combining_function: callable, clip_model: CLIP, preprocess: callable):
    ...
    Perform retrieval on ImgRes validation set computing the metrics. To combine the features the `combining_function`
    is used
    :param combining_function:function which takes as input (image_features, text_features) and outputs the combined
                            features
    :param clip_model: CLIP model
    ...
    return compute_imgres_val_metrics(relative_val_dataset, clip_model, index_features, index_names,
                                   combining_function)
"""
