...
class ImgResDataset(Dataset):
    """
       ImgResDataset dataset class which manage img restoration data
    ...
    def __init__(self, split: str, mode: str, preprocess: callable, \
            dir_ = base_path / 'ImgRestore_dataset'):
        """
        ...
        with open(self.dir_ / 'captions' / f'cap.{split}.json') as f:
            self.triplets = json.load(f)    # [{'candidate': 'dark/014.png', 'target': 'dark/021.png', 'captions': ['similar degradation dark']}, ...]

        # get a mapping from image name to relative path
        with open(self.dir_ / 'image_splits' / f'split.{split}.json') as f:
            self.name_to_relpath = json.load(f)   # ['/d2/noise+defocus blur/084.png', '/d2/dark+motion blur/041.png', ...]

        print(f"ImgRes {split} dataset in {mode} mode initialized")
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    #reference_image_path = base_path / 'ImgRestore_dataset' / 'images' / 'LQ' / f"{reference_name}"
                    reference_image_path = self.dir_ / f"images/LQ/{reference_name}"
                    #print("reference_image_path: ", reference_image_path)
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_name = self.triplets[index]['target']
                    #target_image_path = base_path / 'ImgRestore_dataset' / 'images' / 'LQ' / f"{target_name}"
                    target_image_path = self.dir_ / f"images/LQ/{target_name}"
                ...
                image_name = self.name_to_relpath[index]
                #image_path = base_path / 'ImgRestore_dataset' / 'images' / 'LQ' / f"{image_name}"
                image_path = self.dir_ / f"images/LQ/{image_name}"
