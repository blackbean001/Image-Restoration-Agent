...
from data_utils import base_path, squarepad_transform, targetpad_transform, CIRRDataset, ImgResDataset, FashionIQDataset
...
from validate import compute_cirr_val_metrics, compute_fiq_val_metrics, compute_imgres_val_metrics


def clip_finetune_imgres(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int,
                       validation_frequency: int, transform: str, save_training: bool, encoder: str, save_best: bool,
                       **kwargs):
    """
    Fine-tune CLIP on the ImgRes dataset using as combining function the image-text element-wise sum
    ...
    training_path: Path = Path(
        base_path / f"models/clip_finetuned_on_imgres_{clip_model_name}_{training_start}")
    ...
    relative_train_dataset = ImgResDataset('train', 'relative', preprocess)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % validation_frequency == 0:
            with experiment.validate():
                if encoder != 'text':
                    val_index_features, val_index_names = extract_index_features(classic_val_dataset, clip_model)
                results = compute_imgres_val_metrics(relative_val_dataset, clip_model, val_index_features, val_index_names, combining_function)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, required=True, help="should be either 'CIRR' or 'fashionIQ' or 'ImgRes'")
    ...
    print('Using dataset: ', args.dataset.lower())
    if args.dataset.lower() not in ['fashioniq', 'cirr', 'imgres']:
        raise ValueError("Dataset should be either 'CIRR' or 'FashionIQ' or 'ImageRes'")
    ...
    elif args.dataset.lower() == 'imgres':
        clip_finetune_imgres(**training_hyper_params)
