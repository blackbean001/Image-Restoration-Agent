...
from data_utils import base_path, squarepad_transform, ImgResDataset, FashionIQDataset, targetpad_transform, CIRRDataset
...
def combiner_training_imgres(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str, combiner_lr: float, batch_size: int, clip_bs: int, validation_frequency: int, transform: str, save_training: bool, save_best: bool, **kwargs):
    """
    Train the Combiner on ImgRes dataset keeping frozen the CLIP model
    ...
    training_path: Path = Path(
        base_path / f"models/combiner_trained_on_imgres_{clip_model_name}_{training_start}")
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                ...
                #if encoder != 'text':
                #    val_index_features, val_index_names = extract_index_features(classic_val_dataset, clip_model)
                results = compute_imgres_val_metrics(relative_val_dataset, clip_model, val_index_features, val_index_names, combiner.combine_features)
...
if __name__ == '__main__':
    ...
    if args.dataset.lower() not in ['fashioniq', 'cirr', 'imgres']:
        raise ValueError("Dataset should be either 'CIRR' or 'FashionIQ or 'ImgRes'")
    ...
    if args.dataset.lower() == 'cirr':
        ...
        combiner_training_imgres(**training_hyper_params)
